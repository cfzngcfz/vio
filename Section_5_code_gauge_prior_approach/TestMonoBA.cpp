...
int main() {
    ...
    // gauge prior approach
    double Wp = 1e+15;
    for (size_t mm = 0; mm < 2; ++mm)
    {
        shared_ptr<EdgeSE3Prior> edge_prior(new EdgeSE3Prior(cameras[mm].twc, cameras[mm].qwc));//把第一帧和第二帧的旋转和平移放进去
        std::vector<std::shared_ptr<Vertex> > edge_prior_vertex;                                //容器edge_prior_vertex存放先验的顶点
        edge_prior_vertex.push_back(vertexCams_vec[mm]);                                        //先验顶点
