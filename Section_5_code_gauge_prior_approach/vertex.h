...
namespace myslam {
    namespace backend {
        ...
        class Vertex {
        public:
            EIGEN_MAKE_ALIGNED_OPERATOR_NEW; //Eigen库数据结构内存对齐
             ...
             */
            explicit Vertex(int num_dimension, int local_dimension = -1); // explicit 声明类构造函数是显示调用的

            virtual ~Vertex();

            /// 返回变量维度
            int Dimension() const; // const constant 的缩写，本意是不变的，不易改变的意思。指定一个语义约束，编译器会强制实施这个约束，允许程序员告诉编译器某值是保持不变的
