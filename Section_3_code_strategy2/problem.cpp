...
namespace myslam {
    namespace backend {
        ...
        bool Problem::Solve(int iterations) {
            ...
            cout << "初始阻尼因子" << currentLambda_ << endl;
            ...
            while (!stop && (iter < iterations)) {
                ...
                for (auto vertex : verticies_) {
                    cout << "cur x = " << vertex.second->Parameters().transpose() << endl;
                }
                ...
                while (!oneStepSuccess)  // 不断尝试 Lambda, 直到成功迭代一步  更新阻尼因子？
                {
                    ...
                    // 第四步，解线性方程 H X = B
                    SolveLinearSystem();     // 求解 ( J_f^T * J_f + mu*I ) * delta x_lm = - J_f^T * f(x)
                    ...
                    /*cout << "\n---- verticies_ before UpdateStates()----" << endl;
                    for (auto vertex : verticies_) {
                        cout << "second.Parameters = " << vertex.second->Parameters().transpose() << endl;
                    }*/
                    // 更新状态量 X = X+ delta_x_
                    // cout << "delta_x_ = " << delta_x_.transpose() << endl;
                    UpdateStates();
        ...
        */
        ...
        bool Problem::IsGoodStepInLM() {
            ...
            /*cout << "---- verticies_ after UpdateStates()----" << endl;
            for (auto vertex : verticies_) {
                cout << "second.Parameters = " << vertex.second->Parameters().transpose() << endl;
            }*/
            ...
            for (auto edge : edges_) {
                edge.second->ComputeResidual();                     // 计算/更新残差，因为 IsGoodStepInLM() 之前执行过 UpdateStates() 即 x_lm += \Delta x_lm, 此时计算的是更新后的残差
                tempChi += edge.second->Chi2();                     // Chi2() return residual_.transpose() * information_ * residual_
            }
            /*cout << "\nx_lm 的残差平方和 = " << currentChi_ << endl;
            cout << "x_lm + \Delta x_lm 的残差平方和 = " << tempChi << endl;*/

            // 计算alpha
            double temp = b_.transpose() * delta_x_;
            double alpha = temp / ((tempChi - currentChi_) / 2 + 2 * temp);
            alpha = std::max(alpha, 0.01);  // 如果alpha极其小, 导致rho无穷接近0，无法更新
            cout << "alpha = " << alpha << endl;


            /*cout << "\n---- verticies_ before RollbackStates()----" << endl;
            for (auto vertex : verticies_) {
                cout << "second.Parameters = " << vertex.second->Parameters().transpose() << endl;
            }*/
            ...
            /*cout << "---- verticies_ after  RollbackStates()----" << endl;
            for (auto vertex : verticies_) {
                cout << "second.Parameters = " << vertex.second->Parameters().transpose() << endl;
            }*/


            //cout << "\ndelta_x_ before * alpha= " << delta_x_.transpose() << endl;
            // VecX delta_x_copy = delta_x_;
            delta_x_ = delta_x_ * alpha;
            //cout << "delta_x_ after  * alpha = " << delta_x_.transpose() << endl;
            ...
            tempChi = 0.0;                                   // 记录 x + \Delta x_{lm} 对应的损失函数/残差平方和
            for (auto edge : edges_) {
                edge.second->ComputeResidual();                     // 计算/更新残差，因为 IsGoodStepInLM() 之前执行过 UpdateStates() 即 x_lm += \Delta x_lm, 此时计算的是更新后的残差
                tempChi += edge.second->Chi2();                     // Chi2() return residual_.transpose() * information_ * residual_
            }
            /*cout << endl;
            cout << tempChi << endl;
            cout << currentChi_ << endl;*/

            // 计算rho (see "L3BundleAdjustment.pdf" eq 10)
            double rho = (currentChi_ - tempChi) / scale;
            cout << "rho = " << rho << endl;

            // 策略 2 (see "The Levenberg-Marquardt algorithm for nonlinear least squares curve-fitting problems.pdf" P4 4.1.1 策略2)
