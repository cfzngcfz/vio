    ...
    namespace backend {
        ...
        bool Problem::Solve(int iterations) {
            ...
            for (auto vertex : verticies_) {
                //cout << "id = " << vertex.second->OrderingId() << endl;
                cout << "x = " << vertex.second->Parameters().transpose() << endl;
            }
            ...
            while (!stop && (iter < iterations)) {
                cout << "\n=========== iter = "<< iter <<" =============== " << endl;
                // std::cout << "iter: " << iter << " , chi= " << currentChi_ << " , 阻尼因子= " << currentLambda_ << std::endl;
                ...
                while (!oneStepSuccess)  // 不断尝试 Lambda, 直到成功迭代一步  更新阻尼因子？
                {
                    ...
                    // 第四步，解线性方程 H X = B
                    SolveLinearSystem();     // 求解 ( J_f^T * J_f + mu*I ) * delta x_lm = - J_f^T * f(x), 得到新的 delta_x_
                    RemoveLambdaHessianLM(); // 对角线减去当前的阻尼因子，因为每次迭代中的阻尼因子是变化的

                    cout << "\n---- verticies_ after SolveLinearSystem()----" << endl;
                    for (auto vertex : verticies_) {
                        //cout << "id = " << vertex.second->OrderingId() << endl;
                        cout << "x = " << vertex.second->Parameters().transpose() << endl;
                    }
                    ...
                    cout << "\n---- verticies_ after UpdateStates()----" << endl;
                    for (auto vertex : verticies_) {
                        //cout << "first = " << vertex.first << endl;
                        //cout << "second->OrderingId = " << vertex.second->OrderingId() << endl;
                        cout << "x = " << vertex.second->Parameters().transpose() << endl;
                    }
                    cout << "delta_x_ = " << delta_x_.transpose() << endl;
                    ...
                    if (oneStepSuccess) {
                        ...
                        cout << "\n---- verticies_ after RollbackStates()----" << endl;
                        for (auto vertex : verticies_) {
                            //cout << "id = " << vertex.second->OrderingId() << endl;
                            cout << "x = " << vertex.second->Parameters().transpose() << endl;
                        }
        ...
        //#endif
            ...
            for (auto& edge : edges_) {
                ...
                edge.second->ComputeJacobians(); // 计算雅可比
        ...
        */
        ...
        bool Problem::IsGoodStepInLM() {
            ...
            // 计算rho的分母
            double scale = 0;
            scale = 0.5 * delta_x_.transpose() * (currentLambda_ * delta_x_ + b_); // 原代码没有*0.5
