    ...
    if (PUB_THIS_FRAME) {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++) {
            ...
            for (unsigned int j = 0; j < FeaturePoints.size(); j++) {
                ...
                feature_points->points.push_back(Vector3d(FeaturePoints[j].x, FeaturePoints[j].y, 1));
                feature_points->id_of_point.push_back(p_id * NUM_OF_CAM + i);
                feature_points->u_of_point.push_back(FeaturePoints[j].x);
                feature_points->v_of_point.push_back(FeaturePoints[j].y);
...
void System::ProcessBackEnd() {
  ...
  while (bStart_backend) {
    ...
    if (measurements.size() > 1) {
      cout << "1 getMeasurements size: " << measurements.size() << " imu sizes: " << measurements[0].first.size() << " feature_buf size: " << feature_buf.size() << " imu_buf size: " << imu_buf.size()
           << endl;
    }
    lk.unlock();
    m_estimator.lock();
    for (auto &measurement : measurements) {
      ...
      if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
        ...
        ofs_pose << fixed << dStamp << " " << p_wi(0) << " " << p_wi(1) << " " << p_wi(2) << " " << q_wi.w() << " " << q_wi.x() << " " << q_wi.y() << " " << q_wi.z() << endl;
      }
    }
    m_estimator.unlock();
  }
...
void System::Draw() {
  ...
  s_cam = pangolin::OpenGlRenderState(pangolin::ProjectionMatrix(1024, 768, 500, 500, 512, 384, 0.1, 1000), pangolin::ModelViewLookAt(-5, 0, 15, 7, 0, 0, 1.0, 0.0, 0.0));

  d_cam = pangolin::CreateDisplay().SetBounds(0.0, 1.0, pangolin::Attach::Pix(175), 1.0, -1024.0f / 768.0f).SetHandler(new pangolin::Handler3D(s_cam));
