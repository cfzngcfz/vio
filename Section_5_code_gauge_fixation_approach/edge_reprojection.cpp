...
namespace myslam {
    namespace backend {
        ...
        void EdgeReprojection::ComputeResidual() {
            double inv_dep_i = verticies_[0]->Parameters()[0];  //当前特征点的逆深度
            //verticies_ 当前边对应的所有顶点 defined in edge.h

            VecX param_i = verticies_[1]->Parameters();         // 第0帧的位姿 from camera frame to world frame
            Qd Qi(param_i[6], param_i[3], param_i[4], param_i[5]);
            Vec3 Pi = param_i.head<3>();

            VecX param_j = verticies_[2]->Parameters();         // 第i帧的位姿 from camera frame to world frame
            Qd Qj(param_j[6], param_j[3], param_j[4], param_j[5]);
            Vec3 Pj = param_j.head<3>();

            Vec3 pts_camera_i = pts_i_ / inv_dep_i;
            // pts_i_: camera 第0个相机/第0帧 in camera frame 对 当前特征点的观测值
            // pts_camera_i: 当前特征点在第0个相机/第0帧的 camera frame 的空间坐标
            Vec3 pts_imu_i = qic * pts_camera_i + tic;
            // qic: 旋转 from camera frame to imu frame
            // tic: 平移 from camera frame to imu frame
            // pts_imu_i: 当前特征点在第0个相机/第0帧的 imu frame 的空间坐标
            Vec3 pts_w = Qi * pts_imu_i + Pi;
            // pts_w: 当前特征点在 world frame 的空间坐标
            Vec3 pts_imu_j = Qj.inverse() * (pts_w - Pj);
            // pts_imu_j: 当前特征点在第j个相机/第j帧的 imu frame 的空间坐标
            Vec3 pts_camera_j = qic.inverse() * (pts_imu_j - tic);
            // pts_camera_j: 当前特征点在第j个相机/第j帧的 camera frame 的空间坐标

            double dep_j = pts_camera_j.z();
            residual_ = (pts_camera_j / dep_j).head<2>() - pts_j_.head<2>();   /// J^t * J * delta_x = - J^t * r
            // (pts_camera_j / dep_j).head<2>(): 第j个相机/第j帧 in camera frame 对 当前特征点的理论值/预测值
            // pts_j_.head<2>(): 第j个相机/第j帧 in camera frame 对 当前特征点的观测值
        ...
        void EdgeReprojection::SetTranslationImuFromCamera(Eigen::Quaterniond& qic_, Vec3& tic_) {
            qic = qic_; // qic: 旋转 from camera frame to imu frame
            tic = tic_; // tic: 平移 from camera frame to imu frame
        }

        void EdgeReprojection::ComputeJacobians() {
            double inv_dep_i = verticies_[0]->Parameters()[0];

            VecX param_i = verticies_[1]->Parameters();  //第0帧位姿
            Qd Qi(param_i[6], param_i[3], param_i[4], param_i[5]);
            Vec3 Pi = param_i.head<3>();

            VecX param_j = verticies_[2]->Parameters();  //第j帧位姿
            Qd Qj(param_j[6], param_j[3], param_j[4], param_j[5]);
            Vec3 Pj = param_j.head<3>();
